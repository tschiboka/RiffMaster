...
describe('POST /', () => {
  ...
  it('should create a new user and send email confirmation link', async () => {
    ...
    User.findOne.mockResolvedValueOnce(null);                      // Mock User.findOne to return null (user does not exist)
    jwt.sign.mockReturnValueOnce('mock-token');                    // Mock jwt.sign to return a token
    Token.prototype.save.mockResolvedValueOnce();                  // Mock Token.save to resolve successfully

    const mockTransporter = {                                      // Mock nodemailer.createTransport to return a transporter
